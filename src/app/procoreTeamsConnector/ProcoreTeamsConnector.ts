 ...
 */
interface IProcoreTeamsConnectorData {
    webhookUrl: string;
    user: string;
    appType: string;
    groupName: string;
...
/**
 * Implementation of the "ProcoreTeamsConnectorConnector" Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/procoreTeamsConnector/config.html")
export class ProcoreTeamsConnector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as IProcoreTeamsConnectorData[]).filter(((c) => {
                    return c.existing;
                })));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as IProcoreTeamsConnectorData[]).map((connector, index) => {
